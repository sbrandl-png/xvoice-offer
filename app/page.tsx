...
// ===== EMAIL HTML (mit Listen vs. Angebotspreis & %-Badge) =====
function buildEmailHtml(params: {
  ...
  lineItems: Array<{
    ...
    quantity: number;
    total: number; // bereits rabattierte Summe
  }>;
  subtotal: number; // Summe rabattiert (netto)
  vatRate: number;
  discountPct: number; // nur für Kompatibilität vorhanden
}) {
  const { customer, salesperson, lineItems, subtotal, vatRate, discountPct } =
    params;

  const discountAmount =
    (Math.max(0, Math.min(100, discountPct || 0)) / 100) * subtotal;
...
function ProductRow({
  ...
  return (
    <div className="grid grid-cols-[minmax(220px,1fr)_110px_160px_120px] items-start gap-4 py-3 border-b last:border-none">
      <div>
  ...
  const offerHtml = useMemo(
    () =>
      buildEmailHtml({
        ...
        subtotal,
        vatRate,
        discountPct: 0, // globaler Rabatt nicht genutzt, behalten für Signatur
      }),
    ...
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      ...
      >
        <div className="grid grid-cols-1 gap-2">
          <div className="grid grid-cols-[minmax(220px,1fr)_110px_160px_120px] gap-4 text-xs uppercase text-muted-foreground pb-2 border-b">
            <div>Produkt</div>
            <div>Listenpreis</div>
