...
/**
 * XVOICE OFFER BUILDER – rabatt pro Position (mit harten Max-Grenzen)
 * - XVPS Menge automatisch (XVPR + XVDV + XVMO)
 * - Rabatt pro Position mit Cap (XVPR/XVDV/XVMO 40%, XVTE 20%, XVCRM 20%, XVF2M 100%, XVPS 0%)
 * - Salutation Select, Sales-Signatur
 * - Stabiles HTML-Email-Template (Kundenadresse Box, Warum-Block links + Client rechts)
 */
...
function fullCustomerAddress(c: Customer) {
  const lines = [c.company || "", c.contact || "", c.street || "", [c.zip, c.city].filter(Boolean).join(" ")].filter(Boolean);
  return lines.join("\n");
}
...
function buildEmailHtml(params: {
  ...
  lineItems: Array<{ sku: string; name: string; desc?: string; price: number; quantity: number; discountPct: number; total: number }>;
  subtotal: number;
  vatRate: number;
}) {
            ...
            ${lineItems.map(li => `
              <tr>
                <td style="${s.td}">
                  ...
                  <div style="${s.pSmall}">${li.sku}${li.discountPct > 0 ? ` · <strong>Rabatt −${li.discountPct}%</strong>` : ""}</div>
                </td>
...
function ProductRow({
  item, qty, onQty, discount, onDiscount, readOnly, helper, maxDiscount
}: {
  ...
  return (
    <div className="grid grid-cols-[minmax(260px,1fr)_100px_140px_120px_120px] items-start gap-4 py-3 border-b last:border-none">
      <div>
      ...
      <div className="flex items-center gap-2">
        <Input
          ...
          value={discount}
          onChange={(e) => {
            const n = Math.max(0, Math.min(maxDiscount, Number(e.target.value || 0)));
            onDiscount(n);
          }}
          className="w-24"
      ...
      <div className="text-right font-semibold tabular-nums">{formatMoney(item.price * (1 - discount / 100) * qty || 0)}</div>
      {helper ? <div className="col-span-5 -mt-2 text-xs text-muted-foreground">{helper} · {capHint}</div> : <div className="col-span-5 -mt-2 text-xs text-muted-foreground">{capHint}</div>}
    </div>
  );
}
...
export default function Page() {
  ...
  // Rabatt je Position (in %)
  const [discountBySku, setDiscountBySku] = useState<Record<string, number>>(Object.fromEntries(CATALOG.map((p) => [p.sku, 0])));
  const [vatRate] = useState(0.19); // fix 19%
  ...
  // XVPS Anzahl automatisch
  const serviceAutoQty = useMemo(() => (qty["XVPR"] || 0) + (qty["XVDV"] || 0) + (qty["XVMO"] || 0), [qty]);

  // Zeilen + Summen
  const lineItems = useMemo(() => {
    return CATALOG
      .filter((p) => (p.sku === "XVPS" ? serviceAutoQty > 0 : (qty[p.sku] || 0) > 0))
      .map((p) => {
        ...
        const lineTotal = p.price * (1 - d / 100) * q;
        return { ...p, quantity: q, discountPct: d, total: lineTotal };
      });
  ...
  // HTML Mail
  const offerHtml = useMemo(() => buildEmailHtml({ customer, salesperson, lineItems, subtotal, vatRate }), [customer, salesperson, lineItems, subtotal, vatRate]);
  ...
  async function handleSendEmail() {
    ...
    try {
      await postJson("/api/send-offer", { meta: { subject }, offerHtml, customer, lineItems, totals: { subtotal, vat: subtotal * vatRate, gross: subtotal * (1 + vatRate) }, salesperson, recipients: [customer.email, salesEmail].filter(Boolean) });
      setSendOk(true);
  ...
  async function handleOrderNow() {
    setSending(true); setError(""); setSendOk(false);
    try {
      await postJson("/api/place-order", { orderIntent: true, offerHtml, customer, lineItems, totals: { subtotal, vat: subtotal * vatRate, gross: subtotal * (1 + vatRate) } });
      setSendOk(true);
    } catch (e: any) { setError(String(e?.message || e)); } finally { setSending(false); }
  }
  function resetAll() {
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      ...
      <Section
        ...
        action={<div className="text-xs opacity-70">USt. fest: 19% · Rabatt pro Position mit Cap</div>}
      >
        <div className="grid grid-cols-1 gap-2">
          <div className="grid grid-cols-[minmax(260px,1fr)_100px_140px_120px_120px] gap-4 text-xs uppercase text-muted-foreground pb-2 border-b">
            <div>Produkt</div>
            <div>Preis</div>
            <div>Menge</div>
            <div>Rabatt %</div>
          ...
          {CATALOG.map((item) => {
            ...
            const maxD = MAX_DISCOUNT[item.sku] ?? 0;
            const onDisc = (v: number) => setDiscountBySku((prev) => ({ ...prev, [item.sku]: v }));
            ...
            return (
              <ProductRow
                ...
                qty={q}
                onQty={onQ}
                discount={Math.max(0, Math.min(maxD, discountBySku[item.sku] || 0))}
                onDiscount={onDisc}
                readOnly={isService}
                helper={helper}
                maxDiscount={maxD}
          ...
          <div className="space-y-2">
            {lineItems.map((li) => (
              <div key={li.sku} className="flex justify-between text-sm">
                <div>{li.quantity}× {li.name} ({li.sku}){li.discountPct > 0 ? ` · Rabatt −${li.discountPct}%` : ""}</div>
