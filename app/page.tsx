...
// --- Minimal UI-Komponenten aus deinem Projekt ---
...
function buildEmailHtml(params: {
  ...
  // Inline-Styles für hohe Mail-Client-Kompatibilität
  const s = {
...
function Section(props: React.PropsWithChildren<{ title: string; action?: React.ReactNode }>) {
...
export default function Page() {
  ...
  const totals = {
    ...
    discountAmount,
    netAfterDiscount,
    vat: netAfterDiscount * vatRate,
    gross: netAfterDiscount * (1 + vatRate),
  } as const;

  const offerHtml = useMemo(
    () => buildEmailHtml({ customer, lineItems, subtotal, vatRate, discountPct }),
    [customer, lineItems, subtotal, vatRate, discountPct]
  );
  ...
  function openPreviewNewTab() {
    // 1) Blob URL (besser für lange HTML)
    try {
  ...
  async function postJson(url: string, payload: any) {
    try {
      ...
      // Fallback für Umgebungen, die POST verbieten (405/UnsupportedHttpVerb)
